describe("delivery infor page", () => {
  it("disables/enables buttons accordingly depending on delivery status", () => {
    let test_ids_for_delivered_deliveries = [1]; // <- Pass a custom id here for an order which you are sure is delivered, for other statuses, the test will fail
    ...
    if (cy.get("body").contains(/Status: delivered/i)) {
